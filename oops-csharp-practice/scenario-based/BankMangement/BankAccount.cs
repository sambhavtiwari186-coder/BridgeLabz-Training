...
namespace BridgeLabzTraining.oops_csharp_practice.scenario_based.bank_account_manager
{
    internal class BankAccount
    {
        ...
        public void GetAccountInfo(string accountNumber)
        {
            ...
            {
                if (Accounts[i, 0].Equals(accountNumber, StringComparison.OrdinalIgnoreCase))
                {
                    Console.WriteLine("---------------------------------------");
                    Console.WriteLine($"Account number -> {Accounts[i, 0]}");
                    Console.WriteLine($"Account user -> {Accounts[i, 1]}");
                    Console.WriteLine($"Account balance -> {Accounts[i, 2]}");
                    Console.WriteLine("---------------------------------------");
                    return;
                }
            }
        ...
        public void SetAccountInfo(string accountNumber)
        {
            ...
            int currentBalance = int.Parse(Accounts[accountIdx, 2]);
            ...
            if (ans == "a")
            {
                ...
                if (amountToBeAdded > 0)
                {
                    ...
                    Accounts[accountIdx, 2] = updatedBalance.ToString();
                    Console.WriteLine("---------------------------------------");
                    Console.WriteLine($"Account number -> {Accounts[accountIdx, 0]}");
                    Console.WriteLine($"Account user -> {Accounts[accountIdx, 1]}");
                    Console.WriteLine($"Updated account balance -> {Accounts[accountIdx, 2]}");
                    Console.WriteLine("---------------------------------------");
                    return;
                }
                else
            ...
            else if (ans == "r")
            {
                ...
                if (amountToBeRemoved > 0 && amountToBeRemoved <= currentBalance)
                {
                    ...
                    Accounts[accountIdx, 2] = updatedBalance.ToString();
                    Console.WriteLine($"Updated balance -> {Accounts[accountIdx, 2]}");
                    return;
                }
                else
                {
        ...
        public void Debit(string accountNumber)
        {
            ...
            if (pinEntered != Accounts[accountIdx, 3])
            {
                Console.WriteLine("Invalid PIN!");
                return;
            }
            ...
            Console.WriteLine($"Account number -> {Accounts[accountIdx, 0]}");
            Console.WriteLine($"Account user   -> {Accounts[accountIdx, 1]}");
            Console.WriteLine($"Account balance-> {Accounts[accountIdx, 2]}");
            Console.WriteLine("---------------------------------------");
            ...
            if (amountToBeRemoved > 0 && amountToBeRemoved <= currentBalance)
            {
                ...
                Console.WriteLine($"Current balance -> {Accounts[accountIdx, 2]}");
                Console.WriteLine("---------------------------------------");
            }
            else
            {
